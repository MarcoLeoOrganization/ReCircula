...
const s = {
  ...
  card: {
    backgroundColor: '#ffffff', border: '1px solid #E0D9CF',
    borderRadius: '12px', boxShadow: '0 4px 24px rgba(45,106,79,0.10)',
    padding: '48px 40px', width: '100%', maxWidth: '440px',
  },
  logoWrap: {
    display: 'flex', flexDirection: 'column' as const,
    alignItems: 'center', marginBottom: '28px', gap: '10px',
  },
  logoIcon: { width: '48px', height: '48px' },
  brand:      { fontSize: '22px', fontWeight: '700', color: '#2D6A4F', letterSpacing: '-0.5px' },
  heading:    { fontSize: '24px', fontWeight: '700', color: '#1C1C1C', marginBottom: '4px', letterSpacing: '-0.3px' },
...
/* ── Tarjeta de rol ───────────────────────────────────────────────────────── */
function RolCard({
  value, selected, onSelect, icon, title, description, color, bgColor, borderColor,
}: {
  value: Rol; selected: boolean; onSelect: () => void;
  ...
  return (
    <button
      ...
      onClick={onSelect}
      style={{
        flex: 1, padding: '16px 14px', borderRadius: '10px',
    ...
    >
      ...
      <div style={{ fontSize: '13px', fontWeight: '700', color: selected ? color : '#1C1C1C', marginBottom: '4px' }}>
        {title}
      </div>
      <div style={{ fontSize: '11px', color: '#6B6B6B', lineHeight: '1.4' }}>
        {description}
      </div>
    </button>
  );
      ...
      <div style={s.page}>
        <div style={s.card}>
          ...
          <div style={s.successBox}>
            ...
            <p style={{ fontSize: '18px', fontWeight: '700', color: '#2D6A4F', marginBottom: '8px' }}>
              ¡Revisa tu correo!
            </p>
            <p style={{ fontSize: '14px', color: '#3D3D3D', lineHeight: '1.5' }}>
              Enviamos un enlace de verificación a <strong>{email}</strong>.
              Haz clic en él para activar tu cuenta de{' '}
  ...
  return (
    <div style={s.page}>
      <div style={s.card}>
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <div style={s.fieldWrap}>
            ...
            <input
              ...
              placeholder={rol === 'REPARADOR_VERIFICADO' ? 'Ej. Taller García / Juan García' : 'María García'}
            />
          </div>
        ...
        <p style={s.footer}>
          ¿Ya tienes cuenta?{' '}
          <Link to="/login" style={s.footerLink}>Inicia sesión</Link>
        </p>
      </div>
    </div>
  );
