...
export const register = (payload: RegisterPayload) =>
  axiosClient.post<MessageResponse>('/identity/register', payload).then((r) => r.data)

/** RF-01.1 — Reenviar enlace de verificación de email */
export const resendVerification = (email: string) =>
  axiosClient.post<MessageResponse>('/identity/resend-verification', { email }).then((r) => r.data)
...
export const forgotPassword = (payload: ForgotPasswordPayload) =>
  axiosClient.post<MessageResponse>('/identity/forgot-password', payload).then((r) => r.data)

/** RF-01.3 — Restablecer contraseña con token del correo */
export const resetPassword = (payload: ResetPasswordPayload) =>
  axiosClient.post<MessageResponse>('/identity/reset-password', payload).then((r) => r.data)

/** RF-01.5 — Logout: invalida el JWT activo en BD */
export const logout = () =>
  axiosClient.post<MessageResponse>('/identity/logout').then((r) => r.data)

/** RF-01.4 — Datos del usuario autenticado */
export const getMe = () => axiosClient.get<AuthUser>('/identity/me').then((r) => r.data)
